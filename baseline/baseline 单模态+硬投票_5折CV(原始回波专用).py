...
################################预处理###############################
def standardize_features(train_df, val_df=None, test_df=None):
    """标准化特征并返回处理后的DataFrame和scaler对象"""
    # 确定需要标准化的列（排除ID、时间戳和标签）
    feature_cols = [col for col in train_df.columns 
                   if col not in ['id', 'timestamp', 'label']]
...
def prepare(data, fit_scaler=None, transform_only=False):
    ...
    '''
    columns_to_drop = ['number', 'label_first']
...
def LGBM(train, val, test):
    ...
    train, val, test, _ = standardize_features(train,val,test)
    ...
    result = pd.DataFrame({
        ...
        'predicted_class': predict_class + 1  # 假设类别从1开始编号
    })
...
def XGB(train, val, test):
    ...
    result = pd.DataFrame({
        'id': test['id'],
        'timestamp': test['timestamp'],
        'predicted_class': predict_class + 1  # 类别从1开始编号
    })
...
df2=pd.read_csv('all_radar_features.csv')  
...
####################################单模##################################
    result=LGBM(train,test,test)
    #result=XGB(train,test,test)
    #result=Cat(train,test,test)
    ...
    for label in [1, 2, 3, 4]:
        correct = ((true_labels == label) & (predicted_labels == label)).sum()
        total = (true_labels == label).sum()
        label_accuracy = correct / total
        print(f"标签 {label} 的 Accuracy: {label_accuracy:.4f}")
    '''
